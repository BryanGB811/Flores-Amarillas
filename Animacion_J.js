...
function animateName() {
    ...
    [...name].forEach((char, i) => {
        ...
        /* El girasol del nombre esconde un botón: mismo aspecto y
           animación que las demás letras, pero es interactivo. */
        if (char === "🌻") {
            const btn = document.createElement("button");
            btn.type = "button";
            btn.id = "gardenSunflowerBtn";
            btn.className = "letter sunflower-btn";
            btn.textContent = char;
            btn.style.setProperty("--i", i);
            btn.setAttribute("aria-label", "Detalle especial");
...
/* =========================================================
   MODAL SORPRESA DEL GIRASOL
   ========================================================= */

function setupSunflowerModal() {

    const trigger = document.getElementById("gardenSunflowerBtn");
    if (!trigger) return;

    /* Cambia esta URL por el enlace real (lista de deseos, catálogo, etc.) */
    const SURPRISE_LINK = "https://hallys.pe/producto/ramo-de-3-girasoles/?_gl=1*18ppv19*_up*MQ..*_gs*MQ..&gclid=EAIaIQobChMIhMi2iKP_lgMV9lZIAB0vmCDzEAQYAyABEgIPgfD_BwE&gbraid=0AAAAA9pWW8muBVAA3EeOjldOgS-uDJFhW#";

    const overlay = document.createElement("div");
    overlay.className = "sun-modal-overlay";
    overlay.id = "sunModalOverlay";
    overlay.innerHTML = `
        <div class="sun-modal" role="dialog" aria-modal="true" aria-labelledby="sunModalMsg">
            <p class="sun-modal-icon" aria-hidden="true">🌻</p>
            <p class="sun-modal-text" id="sunModalMsg">
                No creí que te darías cuenta, bueno para que no pienses que soy avaro,
                si quieres te puedes pedir este u otro ramo de
                <a href="${SURPRISE_LINK}" target="_blank" rel="noopener" class="sun-modal-link">aquí</a>
                y mandar captura.
            </p>
            <div class="sun-modal-actions">
                <button type="button" class="sun-modal-btn cancel" id="sunModalCancel">Cancelar</button>
                <button type="button" class="sun-modal-btn accept" id="sunModalAccept">Aceptar</button>
            </div>
        </div>
    `;
    ...
    document.getElementById("sunModalCancel").addEventListener("click", closeModal);

    document.getElementById("sunModalAccept").addEventListener("click", () => {
        window.open(SURPRISE_LINK, "_blank", "noopener");
        closeModal();
    });
